...
# Defina os parâmetros da consulta
username <- "'Tainá Couto'"
changeset_id <- 88428468
bbox_info <- paste0(YM_Dados[YM_Dados$id == changeset_id, "min_lat"], ",", 
                    YM_Dados[YM_Dados$id == changeset_id, "min_lon"], ",", 
                    YM_Dados[YM_Dados$id == changeset_id, "max_lat"], ",", 
                    YM_Dados[YM_Dados$id == changeset_id, "max_lon"])
...
get_user_info <- function(username) {
  ...
  erroneous_urls <- data.frame(url = character(), stringsAsFactors = FALSE)
  ...
  for (user in username) {
    ...
    resposta <- tryCatch(GET(url), error = function(e) {

      erroneous_urls <- add_row(erroneous_urls, url = url)
  ...
  # Retornar o dataframe e a tabela de URLs erradas

  return(list(dados_df = dados_df, erroneous_urls = erroneous_urls))
...
YM_Dados_df <- YM_Dados$dados_df

erroneous_urls <- YM_Dados$erroneous_urls
```
...
if (length(resultados) > 0) {
  ...
  # Iterar sobre cada linha do dataframe YM_Changesets
  for (i in 1:nrow(YM_Changesets)) {
    cat("Processando linha", i, "do dataframe YM_Changesets...\n")
    ...
    # Verificar se elements_tags não é nulo
    if (!is.null(elements_tags)) {
      cat("elements_tags não é nulo para a linha", i, "\n")
      ...
      # Combinar elements_tags_df com o dataframe YM_Changesets
      YM_Changesets <- cbind(YM_Changesets, elements_tags_df)
      cat("Dataframe elements_tags_df combinado com sucesso com o dataframe YM_Changesets.\n")
    } else {
      cat("elements_tags é nulo para a linha", i, "\n")
    }
  }
...
get_user_info <- function(username, save_path = "./") {
  dados_brutos <- list()
  ...
  for (user in username) {
    ...
    save_file <- paste0(save_path, user, "_changesets.json")
    cat("Salvando o arquivo:", save_file, "\n")
    writeLines(conteudo, save_file)
...
# Chamar a função get_user_info com os nomes de usuário e o diretório de salvamento
YM_Dados <- get_user_info(usernames, save_directory)
...
# Função para consultar a API de Edições do OpenStreetMap para um usuário específico
get_user_info <- function(username, save_path = "./") {
  dados_brutos <- list()
...
# Chamar a função get_user_info com os nomes de usuário e especificar o diretório de salvamento
save_directory <- "./data/"
YM_Dados <- get_user_info(usernames, save_directory)
...
get_user_info <- function(username) {
  ...
  for (user in username) {
    ...
    # Verificar se a coluna "tags" está presente e não está vazia
    if ("tags" %in% names(dados_json$changesets) && length(dados_json$changesets$tags) > 0) {
      # Converter o dataframe "tags" em um dataframe do R
      tags_df <- as.data.frame(dados_json$changesets$tags)
      
      # Renomear as colunas, se desejar
      # colnames(tags_df) <- c("nova_coluna_1", "nova_coluna_2", ...)
      
      # Aninhar as colunas do dataframe "tags" dentro do dataframe principal
      dados_json$changesets <- bind_cols(dados_json$changesets, tags_df)
    }
...
get_user_info <- function(username) {
  ...
  for (user in username) {
    ...
    # Verificar se a coluna "tags" está presente e não está vazia
    if ("tags" %in% names(dados_json$changesets) && length(dados_json$changesets$tags) > 0) {
      # Converter o dataframe "tags" em uma lista de dataframes
      tags_dfs <- map(dados_json$changesets$tags, as.data.frame)
      
      # Combinar todos os dataframes em uma lista em um único dataframe
      tags_df <- bind_cols(tags_dfs)
      
      # Renomear as colunas, se necessário
      # colnames(tags_df) <- c("nova_coluna_1", "nova_coluna_2", ...)
      
      # Aninhar as colunas do dataframe "tags" dentro do dataframe principal
      dados_json$changesets <- cbind(dados_json$changesets, tags_df)
    }
